  ...
  return (
    <header className="sticky top-0 z-40 bg-[#0d0d0d]/90 backdrop-blur border-b border-[#1a1a1a]">
      <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
        <a href="/" className="text-white font-bold text-lg tracking-tight hover:text-[#c8a97e] transition-colors">Sonder</a>
            ...
            <div className="relative">
              ...
              {menuOpen && (
                <div className="absolute right-0 top-full mt-2 w-44 bg-[#141414] border border-[#2a2a2a] rounded-xl overflow-hidden shadow-xl z-50">
                  <a href={`/profile/${profile.username}`} className="block px-4 py-2.5 text-sm text-[#ccc] hover:bg-[#1e1e1e] hover:text-white transition-colors">Il mio profilo</a>
                  <button onClick={handleSignOut} className="block w-full text-left px-4 py-2.5 text-sm text-[#888] hover:bg-[#1e1e1e] hover:text-red-400 transition-colors">Esci</button>
                </div>
              )}
            </div>
          ) : (
            <a href="/sign-in" className="text-sm text-[#c8a97e] hover:text-[#e8c99e] transition-colors">Accedi</a>
          )}
        </div>
      </div>
    </header>
